...
import 'package:mobile_projek/registrasiPemilik.dart';
import 'package:shared_preferences/shared_preferences.dart';

class LoginPemilik extends StatefulWidget {
  const LoginPemilik({Key? key}) : super(key: key);

  @override
  _LoginPemilikState createState() => _LoginPemilikState();
}

class _LoginPemilikState extends State<LoginPemilik> {
  ...
  Future<void> loginUser(
    ...
    final response = await http.post(
      Uri.parse('http://10.0.2.2:8000/api/loginpemilik'),
    ...
    if (response.statusCode == 200) {
      ...
      final String idPemilik = responseData['id_pemilik'].toString();

      SharedPreferences prefs = await SharedPreferences.getInstance();
      await prefs.setString('token', token);
      await prefs.setString('id_pemilik', idPemilik);
                          ...
                          TextButton(
                            onPressed: () {
                              Navigator.push(
                                context,
                                MaterialPageRoute(
                                  builder: ((context) => RegistrasiPemilik()),
                                ),
                              );
                            },
